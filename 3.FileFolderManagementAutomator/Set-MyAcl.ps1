...
process {
    try {
        $Acl = (Get-Item $Path).GetAccessControl('Access') # GetAccessControl provides more info than Get-Acl
        $Ar = New-Object System.Security.AccessControl.FileSystemAccessRule($Identity, $Right, $InheritanceFlags, $PropagationFlags, $Type)
        $Acl.GetAccessRules($Ar)
        Set-Acl $Path $Acl
    } catch {
        Write-Error -Message "Error: $($_.Exception.Message) - Linr Number: $($_.InvocationInfo.ScriptNumber)"
    }
}
