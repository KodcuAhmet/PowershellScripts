...
process {
    try {
        # Get today's date
        $Today = Get-Date
        # Find all files in the folder path that has a LastWriteTime earlier than $Age days ago
        ...
        if (-not $Files) {
            ...
            foreach ($File in $Files) {
                ...
                # If the file doesn't already exist in the archive path, create a dummy file to create the folder sturcutre
                if (!(Test-Path -Path $DestinationFilePath -PathType Leaf)) {
                    Write-Verbose "The destination file path $DestinationFilePath does not exist..."
